...
module Vapi
  ...
  # @return [Object] 
    ...
    def initialize(provider:, team_id:, region:, api_key:, id:, org_id:, created_at:, updated_at:, name: OMIT, additional_properties: nil)
      @provider = provider
      @team_id = team_id
      @region = region
      @api_key = api_key
      ...
      @_field_set = { "provider": provider, "teamId": team_id, "region": region, "apiKey": api_key, "id": id, "orgId": org_id, "createdAt": created_at, "updatedAt": updated_at, "name": name }.reject do | _k, v |
  v == OMIT
end
    end
