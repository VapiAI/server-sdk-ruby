...
require_relative "query_tool_messages_item"
...
module Vapi
  class QueryTool
  # @return [Array<Vapi::QueryToolMessagesItem>] These are the messages that will be spoken to the user as the tool is running.
...
#  `messages[].conditions` matches the "reason" argument.
  ...
  # @return [Object] 
    ...
    # @param messages [Array<Vapi::QueryToolMessagesItem>] These are the messages that will be spoken to the user as the tool is running.
#  For some tools, this is auto-filled based on special fields like
#  `tool.destinations`. For others like the function tool, these can be custom
#  configured.
...
#  `messages[].conditions` matches the "reason" argument.
    ...
    # @return [Vapi::QueryTool]
    def initialize(messages: OMIT, knowledge_bases: OMIT, id:, org_id:, created_at:, updated_at:, function: OMIT, additional_properties: nil)
      ...
      @_field_set = { "messages": messages, "knowledgeBases": knowledge_bases, "id": id, "orgId": org_id, "createdAt": created_at, "updatedAt": updated_at, "function": function }.reject do | _k, v |
  v == OMIT
end
    end
# Deserialize a JSON object to an instance of QueryTool
  ...
  item = item.to_json
  Vapi::QueryToolMessagesItem.from_json(json_object: item)
end
