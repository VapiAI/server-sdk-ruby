...
module Vapi
  ...
  # @return [Array<Vapi::GoogleModelToolsItem>] These are the tools that the assistant can use during the call. To use existing
#  tools, use `toolIds`.
...
#  @default 0
  ...
  # @return [Object] 
    ...
    # @param tools [Array<Vapi::GoogleModelToolsItem>] These are the tools that the assistant can use during the call. To use existing
#  tools, use `toolIds`.
#  Both `tools` and `toolIds` can be used together.
...
#  @default 0
    ...
    def initialize(messages: OMIT, tools: OMIT, tool_ids: OMIT, knowledge_base: OMIT, knowledge_base_id: OMIT, model:, realtime_config: OMIT, temperature: OMIT, max_tokens: OMIT, emotion_recognition_enabled: OMIT, num_fast_turns: OMIT, additional_properties: nil)
      ...
      @_field_set = { "messages": messages, "tools": tools, "toolIds": tool_ids, "knowledgeBase": knowledge_base, "knowledgeBaseId": knowledge_base_id, "model": model, "realtimeConfig": realtime_config, "temperature": temperature, "maxTokens": max_tokens, "emotionRecognitionEnabled": emotion_recognition_enabled, "numFastTurns": num_fast_turns }.reject do | _k, v |
  v == OMIT
end
    end
# Deserialize a JSON object to an instance of GoogleModel
  ...
  Vapi::GoogleModelToolsItem.from_json(json_object: item)
      ...
      unless parsed_json["realtimeConfig"].nil?
