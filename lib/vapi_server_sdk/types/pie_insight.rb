...
require_relative "pie_insight_group_by"
require_relative "pie_insight_queries_item"
require "date"
require "ostruct"
require "json"
...
module Vapi
  class PieInsight
    ...
    # @return [Vapi::PieInsightGroupBy] This is the group by column for the insight when table is `call`.
    #  These are the columns to group the results by.
    #  All results are grouped by the time range step by default.
    attr_reader :group_by
    # @return [Array<Vapi::PieInsightQueriesItem>] These are the queries to run to generate the insight.
    attr_reader :queries
    ...
    # @param group_by [Vapi::PieInsightGroupBy] This is the group by column for the insight when table is `call`.
    #  These are the columns to group the results by.
    #  All results are grouped by the time range step by default.
    # @param queries [Array<Vapi::PieInsightQueriesItem>] These are the queries to run to generate the insight.
    ...
    # @return [Vapi::PieInsight]
    def initialize(queries:, id:, org_id:, created_at:, updated_at:, name: OMIT, formulas: OMIT, time_range: OMIT,
                   group_by: OMIT, additional_properties: nil)
    ...
    # Deserialize a JSON object to an instance of PieInsight
    #
    ...
    def self.from_json(json_object:)
      ...
      queries = parsed_json["queries"]&.map do |item|
        item = item.to_json
        Vapi::PieInsightQueriesItem.from_json(json_object: item)
      end
      id = parsed_json["id"]
    ...
    # Serialize an instance of PieInsight to a JSON object
    #
    # @return [String]
    def to_json(*_args)
      @_field_set&.to_json
    ...
    def self.validate_raw(obj:)
      ...
      obj.group_by&.is_a?(Vapi::PieInsightGroupBy) != false || raise("Passed value for field obj.group_by is not the expected type, validation failed.")
