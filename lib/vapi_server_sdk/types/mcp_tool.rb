...
module Vapi
  class McpTool
    ...
    attr_reader :async
    # @return [Array<Vapi::McpToolMessagesItem>] These are the messages that will be spoken to the user as the tool is running.
    ...
    #  want assistant to respond with the result from your server.
    #  Defaults to synchronous (`false`).
    # @param messages [Array<Vapi::McpToolMessagesItem>] These are the messages that will be spoken to the user as the tool is running.
    #  For some tools, this is auto-filled based on special fields like
    #  `tool.destinations`. For others like the function tool, these can be custom
    #  configured.
    ...
    # Deserialize a JSON object to an instance of McpTool
    #
    ...
    def self.from_json(json_object:)
      ...
      messages = parsed_json["messages"]&.map do |item|
        item = item.to_json
        Vapi::McpToolMessagesItem.from_json(json_object: item)
      end
      id = parsed_json["id"]
    ...
    # Serialize an instance of McpTool to a JSON object
    #
    # @return [String]
    def to_json(*_args)
      @_field_set&.to_json
