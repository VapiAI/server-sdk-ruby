...
require_relative "bar_insight_group_by"
require_relative "bar_insight_queries_item"
require "date"
require "ostruct"
require "json"
...
module Vapi
  class BarInsight
    ...
    # @return [Vapi::BarInsightGroupBy] This is the group by column for the insight when table is `call`.
    #  These are the columns to group the results by.
    #  All results are grouped by the time range step by default.
    attr_reader :group_by
    # @return [Array<Vapi::BarInsightQueriesItem>] These are the queries to run to generate the insight.
    attr_reader :queries
    ...
    # @param group_by [Vapi::BarInsightGroupBy] This is the group by column for the insight when table is `call`.
    #  These are the columns to group the results by.
    #  All results are grouped by the time range step by default.
    # @param queries [Array<Vapi::BarInsightQueriesItem>] These are the queries to run to generate the insight.
    ...
    # Deserialize a JSON object to an instance of BarInsight
    #
    ...
    def self.from_json(json_object:)
      ...
      queries = parsed_json["queries"]&.map do |item|
        item = item.to_json
        Vapi::BarInsightQueriesItem.from_json(json_object: item)
      end
      id = parsed_json["id"]
    ...
    # Serialize an instance of BarInsight to a JSON object
    #
    # @return [String]
    def to_json(*_args)
      @_field_set&.to_json
    ...
    def self.validate_raw(obj:)
      ...
      obj.group_by&.is_a?(Vapi::BarInsightGroupBy) != false || raise("Passed value for field obj.group_by is not the expected type, validation failed.")
