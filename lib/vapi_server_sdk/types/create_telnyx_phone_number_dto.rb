...
require_relative "create_telnyx_phone_number_dto_fallback_destination"
require_relative "create_telnyx_phone_number_dto_hooks_item"
...
module Vapi
  class CreateTelnyxPhoneNumberDto
    # @return [Vapi::CreateTelnyxPhoneNumberDtoFallbackDestination] This is the fallback destination an inbound call will be transferred to if:
    ...
    # @return [Array<Vapi::CreateTelnyxPhoneNumberDtoHooksItem>] This is the hooks that will be used for incoming calls to this phone number.
    attr_reader :hooks
    ...
    # @param fallback_destination [Vapi::CreateTelnyxPhoneNumberDtoFallbackDestination] This is the fallback destination an inbound call will be transferred to if:
    #  1. `assistantId` is not set
    #  2. `squadId` is not set
    #  3. and, `assistant-request` message to the `serverUrl` fails
    #  If this is not set and above conditions are met, the inbound call is hung up
    #  with an error message.
    # @param hooks [Array<Vapi::CreateTelnyxPhoneNumberDtoHooksItem>] This is the hooks that will be used for incoming calls to this phone number.
    ...
    # @return [Vapi::CreateTelnyxPhoneNumberDto]
    def initialize(number:, credential_id:, fallback_destination: OMIT, hooks: OMIT, name: OMIT, assistant_id: OMIT,
                   workflow_id: OMIT, squad_id: OMIT, server: OMIT, additional_properties: nil)
    ...
    def self.from_json(json_object:)
      ...
      if parsed_json["fallbackDestination"].nil?
        ...
        fallback_destination = Vapi::CreateTelnyxPhoneNumberDtoFallbackDestination.from_json(json_object: fallback_destination)
      end
      hooks = parsed_json["hooks"]&.map do |item|
        item = item.to_json
        Vapi::CreateTelnyxPhoneNumberDtoHooksItem.from_json(json_object: item)
      end
    ...
    def self.validate_raw(obj:)
      obj.fallback_destination.nil? || Vapi::CreateTelnyxPhoneNumberDtoFallbackDestination.validate_raw(obj: obj.fallback_destination)
