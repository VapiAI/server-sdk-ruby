...
#  voice provider fails.
    ...
    def initialize(caching_enabled: OMIT, chunk_plan: OMIT, server:, fallback_plan: OMIT, additional_properties: nil)
      @caching_enabled = caching_enabled if caching_enabled != OMIT
      @chunk_plan = chunk_plan if chunk_plan != OMIT
      @server = server
      @fallback_plan = fallback_plan if fallback_plan != OMIT
      @additional_properties = additional_properties
      @_field_set = { "cachingEnabled": caching_enabled, "chunkPlan": chunk_plan, "server": server, "fallbackPlan": fallback_plan }.reject do | _k, v |
  v == OMIT
end
    end
