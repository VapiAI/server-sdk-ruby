...
require_relative "telnyx_phone_number_fallback_destination"
require_relative "telnyx_phone_number_hooks_item"
require "date"
require_relative "telnyx_phone_number_status"
...
module Vapi
  class TelnyxPhoneNumber
    # @return [Vapi::TelnyxPhoneNumberFallbackDestination] This is the fallback destination an inbound call will be transferred to if:
    ...
    # @return [Array<Vapi::TelnyxPhoneNumberHooksItem>] This is the hooks that will be used for incoming calls to this phone number.
    attr_reader :hooks
    ...
    # @return [Vapi::TelnyxPhoneNumberStatus] This is the status of the phone number.
    attr_reader :status
    ...
    # @param fallback_destination [Vapi::TelnyxPhoneNumberFallbackDestination] This is the fallback destination an inbound call will be transferred to if:
    #  1. `assistantId` is not set
    #  2. `squadId` is not set
    #  3. and, `assistant-request` message to the `serverUrl` fails
    #  If this is not set and above conditions are met, the inbound call is hung up
    #  with an error message.
    # @param hooks [Array<Vapi::TelnyxPhoneNumberHooksItem>] This is the hooks that will be used for incoming calls to this phone number.
    ...
    # @param status [Vapi::TelnyxPhoneNumberStatus] This is the status of the phone number.
    ...
    # @return [Vapi::TelnyxPhoneNumber]
    def initialize(id:, org_id:, created_at:, updated_at:, number:, credential_id:, fallback_destination: OMIT, hooks: OMIT, status: OMIT,
                   name: OMIT, assistant_id: OMIT, workflow_id: OMIT, squad_id: OMIT, server: OMIT, additional_properties: nil)
    ...
    def self.from_json(json_object:)
      ...
      if parsed_json["fallbackDestination"].nil?
        ...
        fallback_destination = Vapi::TelnyxPhoneNumberFallbackDestination.from_json(json_object: fallback_destination)
      end
      hooks = parsed_json["hooks"]&.map do |item|
        item = item.to_json
        Vapi::TelnyxPhoneNumberHooksItem.from_json(json_object: item)
      end
      id = parsed_json["id"]
    ...
    def self.validate_raw(obj:)
      obj.fallback_destination.nil? || Vapi::TelnyxPhoneNumberFallbackDestination.validate_raw(obj: obj.fallback_destination)
      ...
      obj.status&.is_a?(Vapi::TelnyxPhoneNumberStatus) != false || raise("Passed value for field obj.status is not the expected type, validation failed.")
