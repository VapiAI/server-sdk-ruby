...
#  live call updates like transfers and hangups.
    ...
    def initialize(fallback_destination: OMIT, hooks: OMIT, id:, org_id:, created_at:, updated_at:, status: OMIT, name: OMIT, assistant_id: OMIT, workflow_id: OMIT, squad_id: OMIT, server: OMIT, number:, credential_id:, additional_properties: nil)
      ...
      @_field_set = { "fallbackDestination": fallback_destination, "hooks": hooks, "id": id, "orgId": org_id, "createdAt": created_at, "updatedAt": updated_at, "status": status, "name": name, "assistantId": assistant_id, "workflowId": workflow_id, "squadId": squad_id, "server": server, "number": number, "credentialId": credential_id }.reject do | _k, v |
  v == OMIT
end
    end
