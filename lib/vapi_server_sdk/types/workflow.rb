...
require_relative "workflow_transcriber"
require_relative "workflow_voice"
require_relative "langfuse_observability_plan"
require_relative "workflow_background_sound"
require_relative "workflow_credentials_item"
require "date"
require "date"
...
module Vapi
  ...
  # @return [Array<Vapi::WorkflowNodesItem>] 
    attr_reader :nodes
  # @return [Vapi::WorkflowTranscriber] This is the transcriber for the workflow.
#  This can be overridden at node level using `nodes[n].transcriber`.
    attr_reader :transcriber
  # @return [Vapi::WorkflowVoice] This is the voice for the workflow.
...
#  Currently, only Langfuse is supported.
  ...
  # @return [Vapi::WorkflowBackgroundSound] This is the background sound in the call. Default for phone calls is 'office'
#  and default for web calls is 'off'.
#  You can also provide a custom sound by providing a URL to an audio file.
    attr_reader :background_sound
  # @return [Array<Vapi::WorkflowCredentialsItem>] These are dynamic credentials that will be used for the workflow calls. By
...
#  subset using this.
  ...
  # @return [Object] 
    ...
    # @param transcriber [Vapi::WorkflowTranscriber] This is the transcriber for the workflow.
#  This can be overridden at node level using `nodes[n].transcriber`.
    # @param voice [Vapi::WorkflowVoice] This is the voice for the workflow.
#  This can be overridden at node level using `nodes[n].voice`.
    # @param observability_plan [Vapi::LangfuseObservabilityPlan] This is the plan for observability of workflow's calls.
#  Currently, only Langfuse is supported.
    # @param background_sound [Vapi::WorkflowBackgroundSound] This is the background sound in the call. Default for phone calls is 'office'
#  and default for web calls is 'off'.
#  You can also provide a custom sound by providing a URL to an audio file.
    # @param credentials [Array<Vapi::WorkflowCredentialsItem>] These are dynamic credentials that will be used for the workflow calls. By
#  default, all the credentials are available for use in the call but you can
#  supplement an additional credentials using this. Dynamic credentials override
#  existing credentials.
    # @param id [String] 
...
#  subset using this.
    ...
    def initialize(nodes:, transcriber: OMIT, voice: OMIT, observability_plan: OMIT, background_sound: OMIT, credentials: OMIT, id:, org_id:, created_at:, updated_at:, name:, edges:, global_prompt: OMIT, server: OMIT, compliance_plan: OMIT, analysis_plan: OMIT, artifact_plan: OMIT, start_speaking_plan: OMIT, stop_speaking_plan: OMIT, monitor_plan: OMIT, background_speech_denoising_plan: OMIT, credential_ids: OMIT, additional_properties: nil)
      @nodes = nodes
      ...
      @_field_set = { "nodes": nodes, "transcriber": transcriber, "voice": voice, "observabilityPlan": observability_plan, "backgroundSound": background_sound, "credentials": credentials, "id": id, "orgId": org_id, "createdAt": created_at, "updatedAt": updated_at, "name": name, "edges": edges, "globalPrompt": global_prompt, "server": server, "compliancePlan": compliance_plan, "analysisPlan": analysis_plan, "artifactPlan": artifact_plan, "startSpeakingPlan": start_speaking_plan, "stopSpeakingPlan": stop_speaking_plan, "monitorPlan": monitor_plan, "backgroundSpeechDenoisingPlan": background_speech_denoising_plan, "credentialIds": credential_ids }.reject do | _k, v |
  v == OMIT
end
    end
# Deserialize a JSON object to an instance of Workflow
  ...
  Vapi::WorkflowNodesItem.from_json(json_object: item)
      ...
      unless parsed_json["observabilityPlan"].nil?
      ...
      unless parsed_json["backgroundSound"].nil?
  ...
  Vapi::Edge.from_json(json_object: item)
      ...
      unless parsed_json["stopSpeakingPlan"].nil?
      ...
      unless parsed_json["monitorPlan"].nil?
