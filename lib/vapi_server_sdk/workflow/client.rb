...
#  subset using this.
    # @param request_options [Vapi::RequestOptions] 
    # @return [Vapi::Workflow]
    def workflow_controller_update(id:, nodes: nil, transcriber: nil, voice: nil, observability_plan: nil, background_sound: nil, credentials: nil, name: nil, edges: nil, global_prompt: nil, server: nil, compliance_plan: nil, analysis_plan: nil, artifact_plan: nil, start_speaking_plan: nil, stop_speaking_plan: nil, monitor_plan: nil, background_speech_denoising_plan: nil, credential_ids: nil, request_options: nil)
  ...
  req.body = { **(request_options&.additional_body_parameters || {}), nodes: nodes, transcriber: transcriber, voice: voice, observabilityPlan: observability_plan, backgroundSound: background_sound, credentials: credentials, name: name, edges: edges, globalPrompt: global_prompt, server: server, compliancePlan: compliance_plan, analysisPlan: analysis_plan, artifactPlan: artifact_plan, startSpeakingPlan: start_speaking_plan, stopSpeakingPlan: stop_speaking_plan, monitorPlan: monitor_plan, backgroundSpeechDenoisingPlan: background_speech_denoising_plan, credentialIds: credential_ids }.compact
  req.url "#{@request_client.get_url(request_options: request_options)}/workflow/#{id}"
end
      Vapi::Workflow.from_json(json_object: response.body)
    end
